...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import PortAudio_jll on Julia versions older than 1.3!")
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("4494f438-dd4a-57d3-b63d-980d0e6ad8a4"))
platforms = [Pkg.Artifacts.unpack_platform(e, "PortAudio", artifacts_toml) for e in artifacts["PortAudio"]]
...
if best_platform === nothing
    @debug("Unable to load PortAudio; unsupported platform $(triplet(platform_key_abi()))")
else
    # Load the appropriate wrapper
    include(joinpath(@__DIR__, "wrappers", "$(best_platform).jl"))
end

end  # module PortAudio_jll
